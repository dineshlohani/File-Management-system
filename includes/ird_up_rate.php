...
class IrdUpRate
{
	protected static $table_name = "ird_up_rate";
	protected static $db_fields = array('id','up_id','ltr_rate','lpltr_rate','fiscal_id');
	public $id;
	public $up_id;
	public $ltr_rate;
	public $lpltr_rate;
	...
	public static function find_by_up_id_fiscal_id($up_id=0,$fiscal_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where up_id={$up_id} and fiscal_id={$fiscal_id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
	public static function find_by_fiscal_id($id=0)
	{
		global $database;
		return self::find_by_sql("select * from ".self::$table_name. " where fiscal_id={$id}");
