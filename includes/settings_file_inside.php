...
class FileInside
{
	protected static $table_name = "settings_file_inside";
	...
	public function getName($string_id="")
            {
                     ...
                     return $name_plan_type = $plan_type->name;   
            }
	...
	public static function get_topic_no_from_parent_id($parent_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where id='".$parent_id."' limit 1");
		$result= !empty($result_array)? array_shift($result_array) : false;
                return $result->topic_no;
	}
