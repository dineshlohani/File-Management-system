...
class Chalani
{
	protected static $table_name = "chalani";
	protected static $db_fields = array('id','file_id','doc_id','department','document','project','nepali_date','date','firta_miti','firta_date','reciever_name');
	public $id;
	public $file_id;
	public $doc_id;
    public $department;
	...
	public static function find_by_file_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where file_id={$id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
	public static function find_by_params($doc_id=0,$project=0,$reg_no=0,$file_type=0,$file_inside=0)
	{
		global $database;
		...
		if(!empty($reg_no))
		{
			$sql .= ' and reg_no = '.$reg_no;
                ...
                if(!empty($file_inside))
                {
                    $sql .= ' and file_inside = '.$file_inside;
                }
	...
	public static function count_by_doc_id($id)
	{
		global $database;
		$sql = "select count(*) from ".self::$table_name." where doc_id=".$id;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
	public static function count_by_project($id)
	{
		global $database;
		$sql = "select count(*) from ".self::$table_name." where project=".$id;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
